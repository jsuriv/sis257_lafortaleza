# La Fortaleza - Sistema de Gestión de Licorería
...
La Fortaleza es una solución de software empresarial monorepo diseñada específicamente para la comercialización y administración de licores premium. Combina una plataforma administrativa corporativa, un punto de venta ágil para tiendas físicas y un catálogo interactivo de lujo para clientes finales.
...
```
sis257_lafortaleza/
├── backend_lafortaleza/              # Servidor API NestJS
│   ├── src/
...
│
├── frontend_lafortaleza/             # Cliente Web Vue 3
│   ├── src/
...
1. Ingrese a la carpeta del backend:
   ```bash
   cd backend_lafortaleza
   ```
2. Instale las dependencias necesarias:
   ```bash
   npm install
   ```
3. Cree un archivo `.env` en la raíz de `backend_lafortaleza` basándose en el ejemplo disponible y configure sus variables de conexión a base de datos y clave secreta JWT:
   ```env
   DB_HOST=localhost
   DB_PORT=5432
   DB_USERNAME=postgres
   DB_PASSWORD=su_contraseña
   DB_NAME=sis257_lafortaleza
...
- **Proyecto**: Licorería "La Fortaleza"
